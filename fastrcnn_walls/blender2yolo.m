...
save_path = "C:\project_data\fastrcnn_walls\yoloannots";
frames = [{},{}];
...
if exist('progress_log.txt','file')
...
fprintf('%g\tframes remain\n',length(frames))
i =1;
batch_size = 100;
progress_log = fopen('progress_log.txt','a+');
error_log = fopen('error_log.txt','a+');
% progressbar(i/length(frames));
while 1
    fprintf('\n\napproximate completion\t:%.3g%%\n',100*i/length(frames));
    fprintf('time_elapsed: %g\n\n',toc)
    range = [i:i+batch_size-1];
    range(range>length(frames))=[];
    range_data = cell(length(range),3);
    range_fevals = cell(length(range),3);
    for j = 1:length(range)
        range_fevals{j} = parfeval(@parallel_process_frame,1,frames{range(j),1},frames{range(j),2});
    end
    for j = 1:length(range)
        range_data(j,:) = fetchOutputs(range_fevals{j});
        fprintf('%s\t\t%s\n',frames{range(j),1},range_data{j,1})
    end
    for j = 1:length(range)
%         progressbar(range(j)/length(frames))
        switch range_data{j,1}
            case 'complete'
                fprintf(progress_log,'%s\n',frames{range(j),1});  
                image = imread(range_data{j,2}{1});
...
%% functions 
function slice = parallel_process_frame(path_to_frame,savepath)
    dims = [900 1600 3];
    slice = cell(1,3);
    try 
        [slice{2},slice{3}] = process_frame(path_to_frame);   
        shapes2yolo(dims,slice{3},savepath);
        slice{1} = 'complete';
    catch e 
        fprintf('%s\t\terror\n',path_to_frame)
...
function shapes2yolo(dims,shapes,savepath)
    file = fopen(fullfile(savepath,'image.JPEG.txt'),'w');
    for i = 1:size(shapes,1)
        leftbound = min(shapes{i,3}.Vertices(:,1));
        rightbound = max(shapes{i,3}.Vertices(:,1));
%         shapes{i,3}.Vertices(:,2) = dims(1)-shapes{i,3}.Vertices(:,2);
        upperbound = max(shapes{i,3}.Vertices(:,2));
        lowerbound = min(shapes{i,3}.Vertices(:,2));
        width = rightbound-leftbound;
        height = upperbound-lowerbound;
        fprintf(file,'%s\t%g %g %g %g\n',shapes{i,1},leftbound,upperbound,width,height);
    end
    fclose(file);
end
