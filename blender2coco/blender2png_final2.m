...
while 1
    ...
    for j = 1:length(range)
        range_fevals{j} = parfeval(@parallel_process_frame,1,frames{range(j),1} );
    end
    for j = 1:length(range)
...
%         progressbar(range(j)/length(frames))
        switch range_data{j,1}
            case 'complete'
                ...
                shapes2png(dims,range_data{j,3},frames{range(j),2});     
...
%% functions 
function slice = parallel_process_frame(path_to_frame)
    dims = [900 1600 3];
    slice = cell(1,3);
    try 
